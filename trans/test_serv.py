...
def test_google():
    services.translate_google('Hello, how are you?', 'en', 'vi')
...
def test_ins_onyms_from_str_li():
    services.iup_translation(TxtLC('peace', 'EN'), TxtLC('frieden', 'DE'), 'INPUT', 80)
    services.iup_translation(TxtLC('war', 'EN'), TxtLC('krieg', 'DE'), 'INPUT', 80)
    services.iup_translation(TxtLC('conflict', 'EN'), TxtLC('konflikt', 'DE'), 'INPUT', 80)
    services.iup_translation(TxtLC('fight', 'EN'), TxtLC('kampf', 'DE'), 'INPUT', 80)

    ant_li = ['peace', 'war']
    services.ins_onyms_from_str_li('EN', ant_li, 'INPUT', 'ant')
    syn_li = ['war', 'conflict', 'fight']
    services.ins_onyms_from_str_li('EN', syn_li, 'INPUT', 'syn')


def test_find_onyms():
    txt_lc_src = db.fiby_txt_lc(TxtLC('war', 'EN')).result
